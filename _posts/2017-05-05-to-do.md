...
```
Nice! After 50 runs, we find a better solution and these are exactly the categories we expected. This time, the result will be consistent if you re-run the code (unless you're really unlucky). But how do we define better? The $$k$$-means algorithm chooses a solution with lowest inertia:
```python
...
The final step is to find a way to visualize the clusters.
...
[^5]: The sum of squared distances is more of an intuition here, what we're really minimizing is within-cluster variance. They're the same in this case, but we don't want anyone from the Internet to "well, actually" us, do we?
