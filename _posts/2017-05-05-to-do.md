...
Before we can run any machine learning algorithms on text documents, we need to find a way to represent them as numbers. One idea could be to construct a following term-document matrix:
...
Unfortunately, our term-document matrix is not terribly useful. The most frequent words will most likely be *the*, *be*, *to*, *of*, and the like, so the document vectors don't convey a lot of information. What we need is a clever way to discount unimportant terms and bump up relevant ones. Intuitively, a term is relevant to a document if it occurs frequently in a certain document but is rare across all documents, it's probably relevant to that document. Slightly more formally, we want to assign to each term $$t$$ in document $$d$$ a weight $$w(t, d)$$ such that:

* it is proportional to the frequency of $$t$$ in $$d \in D$$,
* it is inversely proportional to the frequency of $$t$$ in the corpus $$D$$.

One of the popular weighting scheme is called tf-idf.
